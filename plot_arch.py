...
def main():
    ...
    p.add_argument("--in_features", type=int, default = 4)
    ...
    with g.subgraph(name="cluster_t") as c:
        c.attr(label="Termination", color=C_BORDER, fontcolor="#334155", style="rounded,dashed", fontsize="12")
        c.attr("node", fillcolor=C_TERM)
        c.node("t_head", f"Head\\n(B,4) → (B,{args.num_options})")
        c.node("t_sig",  f"Sigmoid\\n(B,{args.num_options}) → (B,{args.num_options})", fillcolor=C_CARD)
    ...
    with g.subgraph(name="cluster_p") as c:
        ...
        c.node("p_head", f"Head\\n(B,4) → (B,{args.num_actions})")
        c.node("p_out",  f"Output logits\\n(B,{args.num_actions})", fillcolor=C_CARD)
        c.attr(margin="4")
